...
**September 15, 2026** *(branch `feat/right-sized-database-reads`, commits `c991a63b` + `24db39b5`, unpushed; typecheck / lint 151 (0 errors) / ai:drift:check / ai:matrix:check / data:read:check green; **migration handed off, not applied** — `scripts/handoff/2026-09-15-data-row-digests.{patch,md}`; **production smoke pending**)* — **Right-sized database reads + AI row digests.** `query_database` evolved in place: index tier by default (~65 tok/row), 8-hex row handles on read and write, token budget against the user's threshold (Settings → AI → "Database read approval", default 6k) with an approval card above it, `search` / `rowIds` / `groupBy` / `columns: "all"` / `relations` / `lifetime` (turn · run · chat, folded by `context-diet.ts` with matching chat chips) / `digests`. `describe_database` adds column profiles, sample rows and digest coverage. Charter master ledgers gain standing-context columns (`Reference tables`, `Standing context`, `Context columns`, `Context filter`); the iteration card lists the reads it pins. Row digests: `DataRowDigest` sidecar (hash staleness, dirty-bit discovery), batched generation on the new `row-digest` route under the auto-context daily cap, on-access / nightly / manual triggers, schema-rail switch, row-page field with stale badge. Gates: `pnpm data:read:check` (in `build`, mutation-tested) and `pnpm data:read:measure <tableId>`. Plan + measurements: `work-tracking/AI-BULK-ROW-READING-PLAN.md`; follow-ups in BACKLOG ("Right-sized database reads — follow-ups").

**September 14, 2026** *(branch `chore/status-reconcile-234`, commits `9603fbd4`…`42508bf3`, unmerged; typecheck / lint 151 (0 errors) green; **no migration, no TipTap schema change → no Hocuspocus redeploy**; production write performed by the owner via the loader, verified read-only afterwards)* — **The career evidence ledger became the Career Evidence Library, and the exports that made it possible are product features.** Origin: the owner's 28,103-char "Master career evidence ledger" note (28 accordion sections) needed to land in the four linked tables PR #231 had built — and no route existed: the @mention capsule injects a note's first 2,000 chars, per-item iteration enumerates pages and rows but not a document's sections, and CSV import is a reserved stub. **Shipped in the product:** the markdown and plaintext exports now render accordion / card-panel headers as headings (they were attrs, so both converters dropped them — the vault export of the ledger came out as untitled prose), `statsTable` rows as a table, and block children blank-line separated; `tiptapToMarkdownDocument()` exposes the export rendering to scripts; every database export gains a `.schema.md` sidecar (`lib/domain/data/schema-markdown.ts`, pure) listing columns, option sets and relation edges with resolved names — the "vanilla" schema an outside model can read. **Docs:** `PRODUCT-PRINCIPLES.md` finally landed on a branch (it only existed on `chore/parked-session-work`) and gained **Principle 2 — unrefined → structured is the core loop** with the use-case table (meeting notes, reading notes, research, job hunts, periodic notes, the repo's own tracking docs) and a "gaps are data" rule; BACKLOG carries the feature soil (document-sections iteration, `read_content_chunk`, send-block-to-database + table picker, paste rows, model-aware mention budget, 1M-token note ceiling, accordion codec, deliberate-gap semantics, flashcards → database schema, CSV import). **The migration itself** ran as a protocol in `~/Downloads/career-evidence-migration/PROTOCOL.md`: Claude Code exported a structure-preserving ledger + schema packet + JSON contract; OpenAI reconciled twice against the owner's source documents (its memory contributed nothing — the documents did); the owner answered 22 questions; OpenAI Codex authored the payload; Claude Code reviewed it against an independently built payload (26 "Documented" labels on self-reported text, 28 stamped default dates, ownership under-stated on 11 rows, an inverted relation-pair note) and Codex revised; a read-only prod dry run then caught a Library `Name` collision the contract could not (§20 and §25 collapsed once trailing colons were trimmed). **Loaded 2026-09-14 via `scripts/import-career-evidence.ts`** (owner-run; `createRows` / `writeCells` / `writeRelationLinks` / `updateColumn`, never SQL; upsert on the ID columns; canonical relation pair only): 35 experiences · 66 sources · 99 claims · 35 library rows · 31 options · 1,025 links · 0 errors — after a first attempt died creating 99 claims in one interactive transaction over the Neon pooler (Prisma's 5 s ceiling); the loader now creates in batches of 10 and the resume was a no-op for the two tables already in. Verified read-only: counts, links and backlinks match, every select cell stores an option id. **Surfaced for the product (BACKLOG):** `createRows` single-transaction ceiling, primary-column dedupe in `insert_rows`, and evidence-level definitions in column descriptions — two models read "Documented" differently from the same mapping. **Open:** post-load export bracket, attaching the 4 source docs + 16 screenshots to Source `File` cells, saved gap views, and deleting the now-empty duplicate relation pair.

**September 13, 2026** *(**PR #234**, branch `fix/linked-schema-reciprocal-and-visibility`, merged `ea1b098e`; typecheck / lint 151 (0 errors) / ai:drift:check / full build green, re-verified on main after merge; **no migration, no TipTap schema change → no Hocuspocus redeploy**; **production smoke pending** — charter/database work is verified on prod post-deploy by owner policy)* — **Proposals land where the chat said they would, and a set of tables can say which one is the head.** Origin: the first production run of PR #231's linked-schema tool (conversation `b2e34f16`) plus the owner's review of the result. **The tool itself worked** — one card, three tables and nine relations in a single transaction, no feature-request note — but the run exposed four things. **(1) Both halves of every relation.** The model proposed all six directions between three tables; a relation already mints its mirror, so the spec described four columns where two were meant, and production now carries `Experience` ×2 on Claims and metrics plus a collision-suffixed `Claims and metrics 2` on Experiences (**owner cleanup pending — the fix cannot remove what already exists**). The prompt and both tool descriptions already said to propose one side; nothing enforced it. Now refused in `applyLinkedSchema` (protects every caller, including the batch route) *and* in `propose_linked_databases` (so the model re-proposes inside the same turn instead of failing the user's Apply); self-relations and two distinct same-direction relations stay legal; verified against the recorded payload. **(2) The "missing card" was a stale browser bundle**, not a defect — production had deployed 16 hours before the tab was used, and the card rendered correctly when server-rendered against the exact 8,020-byte production payload. Nothing said so, though: the model announced a review card and the page showed raw JSON. An unrecognized `__<name>Proposal` sentinel now renders an explicit reload notice and suppresses the raw tool bubble, so the next version skew is legible rather than reading as a broken feature. **(3) Databases ignored the Target-output setting.** A chat set to "Under this chat" still got plain siblings: the database tools read only `targetFolderId`, never the output owner, so the control was visible, promised nesting, and was half-applied in silence. They now resolve placement exactly as the note tools do — explicit output folder, then output owner (referenced under the chat/content), then charter folder, then target folder — with the owner riding proposal → card → route and `applyLinkedSchema` stamping `role: "referenced"` + `ownedByNoteId`; a referenced table stores in its owner's folder so path/cascade invariants hold and the move route's detach still works. **Side chats now default to `underContent`** (a side chat is opened ON something; its outputs belong to that content, not the conversation about it), passed as a *mode* so the hydration effect keeps an honest dep array. **Destination labels name the file** — "Under Portfolio Summary.md" / "Beside Portfolio Summary.md", elided at 22 chars, degrading to the generic wording where no title resolves — "Beside" gained its own icon, and the folder picker became a fourth peer option (**"Somewhere else…"**, expanding on click, pre-expanded when a folder is selected, fetching the tree only once opened). **(4) A database can now nest under a database** (owner request): it reuses the reference grammar — referenced, owned by its host, stored in the host's folder, shown behind its chip — with an `ownedByNoteId` cycle guard (bounded walk; `parentId` checks cannot see a reference loop) and `role` restored to `primary` on detach, so a table dragged back to a folder does not hide behind that folder's chip. **Owner decision (agreed 2026-09-13): nesting is NOT restricted to tables that reference each other** — a relation can be deleted later, so a drop-time rule would leave the tree asserting a dead link, and the cascade on relation-delete is the expensive part, not the check. Nesting is organizational the way folders are; relations stay the data fact.

**September 12, 2026** *(**PR #231**, branch `feat/ai-relational-database-reach`, merged `0114a80a`; typecheck / lint 151 (0 errors) / ai:drift:check (65 tools, new gate 6, mutation-tested ×3) / full build green, re-verified on main after PR #230 landed alongside; **no migration, no TipTap schema change → no Hocuspocus redeploy**; **production smoke pending** — charter/database work is verified on prod post-deploy by owner policy)* — **The AI can build relational databases, and it stops reporting its tools' limits as the product's.** Origin: prod conversation `c66c8efd` (2026-09-12, chat bound to "Career Evidence Library"), where the owner asked for three linked career-evidence tables and got four tables, 44 columns, **zero relations** — every cross-table link a text column of hand-typed ids (`EXP-012`) — plus a 427-word feature request note (`3cc169ea`) stating the product had no relation columns, no backlinks, no rollups and no referential integrity. **Four of those five gaps were false**: Phase 4 shipped `relation` (with `DataRowLink` and symmetric backlinks minted by `createRelationPair`), `lookup` and `rollup`, all offered by the grid's own type picker. The model reported them absent because its two proposal tools carried **hand-copied subsets** of `DataColumnType` that never grew, and because `buildDataSchemaDigest` rendered a relation as `- Claims (relation)` with no target — nothing it could read said otherwise. **Six commits:** (1) the digest names the graph (`relation → Claims and metrics, mirrored as "Experience" there`; backlink direction; rollup function and path), resolved in two batched queries and skipped entirely for tables with no graph columns; (2) `applyLinkedSchema` — N new tables, `$new:` sibling references, relation columns onto existing tables, lookups/rollups, **all in one transaction**, with tx-aware `createColumnTx`/`createRelationPairTx` extracted from the existing helpers, and `AI_PROPOSABLE_COLUMN_TYPES` as the single vocabulary (the old `POST /api/content/data` route, previously "atomically-ish", now delegates and gains relations + real atomicity; `phone`, which no picker offers, is no longer creatable); (3) one shared `proposedColumn` Zod schema for every propose_* tool, the new `propose_linked_databases`, `POST /api/content/data/batch`, and **drift gate 6** (proposable list ≡ implemented minus `person`; no tool may re-declare an enum; no create route may keep its own set) — mutation-tested three ways before merge; (4) `LinkedDatabasesProposalCard`, which leads with the **edges** because a relation's mirror lands on a table the user never asked about, and `DatabaseColumnsProposalCard` moved to the batch endpoint, trading its partial-failure state for all-or-nothing; (5) two prompt rules — **"name the TOOL that lacks it"** (a model only sees what it was handed, so it is not positioned to say what the product lacks; feature requests must title each gap by its tool and mark unverified claims) and a relational-database block banning text-ID stand-ins and requiring that a table the user already has be **extended, not rebuilt as an index** of the new ones (the product principle this failure quietly violated); (6) relation cells in `insert_rows`/`update_row` — addressed by target row **title** or id, written after the row exists, replace-not-append on update — which is what makes the owner's next step (migrating the 28k-char "Master career evidence ledger" **note** into the tables) expressible at all. **Authorization moved with the work**: the batch route runs every extend target through the `canAlterSchema` ladder, so a database shared with owner rights can still join a schema; `applyLinkedSchema` executes and no longer adjudicates. **Deliberately NOT built**: `link_rows` (a relation cell's value IS its links — no niche tool), and the capture path still refuses relations explicitly, since it stamps cells through `writeCells` and has no link path.

**September 11, 2026 (night)** *(**PR #227**, branch `feat/shareable-image-links`; typecheck / lint 151 (0 errors) / full build green; **migration `20260911220000_file_payload_public_token`** committed by the owner (additive, nullable) and applied to dev and prod Neon on 2026-09-12; `/f/<token>` smoke-verified on the production build — malformed and unknown tokens 404, a minted token 302s to a one-hour presign and the follow-up fetch returns the PNG; no TipTap node or attribute change → no Hocuspocus redeploy)* — **Copied notes carry images as working public links.** Origin: the owner asked whether copying note content with images could make them transferable by URL to any application that renders image URLs, an AI chat included. Both clipboard flavors were failing: the plain-text serializer had no image case (images vanished from chat inputs), and the HTML flavor carried each image's relative, session-only download path (rich editors showed a broken image). **Fix, three layers:** a lazily minted, rotatable capability token per file (`publicToken`); a no-session `GET /f/<token>` route that 302s to a fresh one-hour presigned storage URL so the bucket stays private while the link stays stable; and a `transformCopied` hook in the Clipboard extension that rewrites image `src` in the outgoing slice only (the document is never touched), fed by a client-side link cache primed as image nodes appear because the copy event is synchronous. The text flavor now emits `![alt](https://<origin>/f/<token>)`. **Stated caveat:** a URL helps only where the destination fetches it — the API surfaces and agentic tools do, the claude.ai and ChatGPT web inputs do not, and the clipboard carries one image blob per write, so those need a pasted file. **Surfaced, not fixed (BACKLOG):** the public site renderer and the markdown export converter still emit the private download path; the R2 provider returns seven-day presigned URLs that the media upload route stores into hero/gallery block attributes.

**September 11, 2026 (evening)** *(**PR #222**, branch `feat/charter-icons-sidechat-quest-ledgers`; typecheck / lint / charters:check / full build green; no migration, no TipTap change, no Hocuspocus redeploy)* — **A charter keeps its markers, every side chat attaches its content, and a named quest row always has its ledger.** Origin: the owner's prod smoke of PR #221 — "Career Hunt II" showed a plain folder icon beside its freshly minted master ledger. Its payload metadata held only word counts: the mark had stamped `charter` and `masterLedgerId`, then the opt-in starter scaffold went through `writeNoteContent`, which upserted a fresh `{wordCount, characterCount, readingTime}` object and wiped both — the same wipe would hit any AI note edit (run-ledger keys, quest config). **Fix, three layers:** `writeNoteContent` now merges prior metadata like the PATCH route and the collaboration store hook; the mark route scaffolds FIRST and stamps last; `ensureMasterLedger` adopts an existing master (referenced to the charter, D6 icon) and re-stamps before it would ever mint a second one — so re-marking Career Hunt II repairs it in place. **Side chats attach their content** (owner: "every side chat should auto-attach the content it exists under"): the bound content rides as an implicit first @-mention in the chat route (body / folder capsule / database digest loaded, its own slot so the user's five mentions are untouched); charters keep their progressive-disclosure path, chats and workflows are their own subject; the rooted-context line now says the content is attached and loaded. **Quest-row hard rule** (owner: "any quest generation should create a system-default ledger, just like charters"): `ensureLedgersForMasterRows` mints or re-links a quest ledger the moment a master-ledger row has a name — hooked into the grid's cell writes (`PATCH /data/[id]/rows`), `insert_rows`, and `update_row`; a cheap two-lookup no-op on any other table; blank rows wait for their name.

**September 11, 2026** *(**PR #221**, branch `feat/charter-ledgers-on-mark`; typecheck / charters:check incl. placeholder detection / lint / build green; **no migration, no TipTap schema change, no Hocuspocus redeploy** — the system-column flag is a `DataColumnConfig` specialization)* — **Charters get their ledgers at mark, a charter's side chat is an attachment, and ledger machinery columns are locked.** Origin: prod conversation "Charter Database Table Design" (2026-09-11), where a chat bound to the Career Hunt I charter asked to "fashion the database for this charter" and got a 20-column orphan table (`propose_output_database`) that nothing reads — the charter's own master ledger was still unminted (lazy D10), the binding counted only as *ambient awareness* (no `activeCharter`), and no prompt string ever named the ledger. **Bound = attached**: `boundCharterId` is resolved once in the chat route and consumed by both the routing resolver and the context block; the ambient tier and its `charterAwareness` prompt field are removed; the engine mirrors the binding in the chip and the persisted part. **Ledger at mark**: `POST /charters/mark` calls `ensureMasterLedger` (idempotent; re-marking backfills older charters), the response carries `masterLedgerId` / `masterLedgerCreated` / `templatePhases`, and the toast says where the ledger is. Folder charters home their ledgers *inside* the folder; quest ledgers are referenced to the charter like the master. **System columns**: `DataColumnConfig.system` on the machinery core — refused in `updateColumn` / `softDeleteColumn` (name/config compared canonically against the live row so an unchanged grid Save passes), surfaced 403 by the column routes, greyed in the column menu with a lock glyph in the header, refused by `propose_column_options`. **Low-hanging gaps folded in**: the output-database tool's phantom 20-column cap now matches the route's 30; the rooted regex accepts "charter"; `add_quest_ledger_column` is back in the mid-run tool diet; `record_item_result` no longer bails on a missing target folder for quest-keyed runs; `ensureQuest` / `recordQuestItem` refusals log and surface (`questWarning`) instead of reporting success; a master row with no live ledger self-heals into a quest; the output-database card keeps the proposal's purpose as the table description; starter placeholder phases are counted (`countStarterPlaceholderPhases`, CI-pinned) and reported at mark and in attached context. **Quests v2 — row-level attachment (owner questions, same day)**: every quest ledger gets a system "Quest" relation to its master row (backlink "Ledger rows"), and one relation per output table it captures into ("Output row · <table>", backlink "Quest · <label>" on the user's table, unlocked) — a relation targets one table, so several output tables mean several columns, minted find-or-create at proposal and recomputed per sitting; the master's "Output table" cell accumulates instead of overwriting; `recordQuestItem` links each item to its quest row and its captured row; v1 ledgers upgrade in place (`QUEST_SCHEMA_VERSION = 2`). **Owner clarification**: the lock is on system-column *definition* only — cells and rows stay open to the grid and the AI (an AI adding a quest row is expected), and a cleared link cell re-links the existing ledger rather than minting a duplicate. **Proposed, not built**: `propose_column_changes` (rename / retype / delete of user columns, system columns rejected) — see EXTRACTION-TO-DATABASE-PLAN §3.6/§10.
**September 11, 2026** *(branch `feat/window-reference-drawer`; typecheck / lint 151 (0 errors) / reference-block:check incl. new window-row block (mutation-tested ×2) / shortcut-mirror:check / full build green; **no migration** (`ContentLink.linkType` is an open varchar), **no TipTap schema change**, but **Hocuspocus redeploy REQUIRED post-merge** — the store hook in `documents.ts` now syncs window-ref edges and ships in that image)*: **Reference Drawer — windowed notes surface as derived shortcut rows**

A note that embeds other notes through Note Window blocks now shows each windowed target in its Reference Drawer, styled like a shortcut (AppWindow corner badge) but **never stored as one**. Rows are derived per tree fetch from new `window-ref` ContentLink edges, so a retargeted or deleted window rewrites its edges on the next save and the row simply stops being synthesized — the staleness that killed the auto-created-Shortcut design is impossible by construction. `syncWindowReferences` (deliberately simpler than image-refs: no ref-count cleanup, no soft-delete — a window points at its target, never owns it) runs on all three write paths, **including the collaboration store hook**, which the media-ref sync never covered; Y.js-first saves are the primary path, so hooking only REST would have left edges stale for exactly the notes users edit most. The tree route appends targets to the host note's `references` array under path-scoped `wref:<hostId>/<targetId>` ids (the same target windowed by two notes — plus its real row — must never share an id in react-arborist), and the rows adopt the shortcut-mirror contract (`mirrorOf` + `isShortcutMirror`), so selection-resolves-to-target, drag refusal, and the read-only context menu all came from existing branches with zero new client transforms. Folder-target window rows are excluded from the mirror chevron (the mirror transform only builds children for shortcut rows — the chevron would open onto nothing) and act as flat pointers. Known scope edges: edges backfill lazily (a note's windows appear after its first post-deploy save), and image-ref edges still skip the collab store path — same latent gap, noted for a follow-up.
...
**D1 — a turn could end silently.** `stopWhen: stepCountIs(n)` is a guillotine, not a budget: it cuts *after* the step boundary, so a turn whose last step emitted tool calls terminates holding tool results with no synthesis step and nothing rendered. The `step-cap-hit` flag was already computed and persisted by `turn-diagnostics.ts` — and read by **nothing**, so the failure was fully diagnosed server-side and completely invisible to the user. `prepareStep` now reserves the final step for prose (`toolChoice: "none"`) plus a harness notice telling the model the loop is over and forbidding any claim a tool result doesn't support — a model whose tools vanish without explanation narrates success it never achieved, and a confabulated answer is strictly worse than the silence being fixed. Because a reserved step finishes `"stop"`, it would have blinded the very diagnostic it exists to surface; `TurnSegment.finalStepReserved` carries the signal so `step-cap-hit` still fires (mutation-tested).

**D2 — `searchNotes` → `search_content`.** The old finder was hard-scoped to `contentType: "note"`, so databases, folders and files were invisible to every search the model could make. `DataPayload.searchText` (table title + column names) had existed for exactly this since the Database work and was never read. The replacement searches notes/folders/databases by default, takes an optional `types` filter, labels each hit with its type, id and parent folder so the model can pick the right follow-up tool, and — the other half of the fix — **lists** when the query is empty: matching cannot answer "which databases exist?", and with no way to enumerate the model had fallen back to guessing table names one call at a time. The database refusals in `resolve.ts` now name `search_content` instead of advising "ask the user", which the model had ignored four guesses running. `toolConfig` carries the old key forward so a deliberately-disabled finder is not silently re-enabled under its new name.

**D3 — `propose_database_columns`.** The toolkit could read a database, write its cells, and mint an entirely new one, but could not add a column to a table that already existed (`add_quest_ledger_column` is fenced to active quest ledgers by design). The request was unanswerable by construction. The new tool diffs the wanted schema against the live one — columns already present are reported, never re-proposed, so the model can send the full wanted set — and renders `DatabaseColumnsProposalCard`, whose Apply POSTs each column. Deliberately **add-only**: rename/retype/remove are a different risk class (a retype can invalidate every cell) and stay the user's own action in the grid. Because Apply spans several requests, a mid-way failure reports a **partial** result naming the columns that failed; half a schema applied but reported as done would leave the user unable to tell what to finish by hand.

**D5 — promotion minted the empty charter, then promised it was ready.** The owner correctly pushed back on the first write-up's "empty folder": the promotion *had* happened (`metadata.charter = true` with a description). What was empty was the charter's BODY — and the mark route is what made it so. A folder carries no `NotePayload` row until its Notes editor first saves, so marking one takes the route's `create` branch, which writes `tiptapJson: {type:"doc",content:[]}`. The promotion mints the blank document, and the dialog then toasted "Marked as charter — attach it from any chat with /charter", promising a readiness a bodyless charter does not have (attaching one reports "contains no instructions"). Two surfaces knew and both hid it: the route had just written the document, and the `/charter` picker renders `description || "N phases"` — so a described-but-blank charter looked identical to a working one at the exact moment of attaching. The route now returns `hasBody`/`phaseCount`, the dialog warns instead of congratulating, and `listCharters` carries `hasBody` (NOT derivable from `phaseCount`: a charter of pure standing rules with no `##` headings is valid) so the picker labels an empty charter as empty. Reported, never blocked — marking first and writing after is a legitimate order of work.

**D5a — the starter template (owner: "a sort of template — yes, absolutely").** Warning someone that their charter is empty is honest but still leaves them on a blank page, so promotion now offers a starter body: standing rules, a `## Context` section, and two phases carrying `Done when:` stop conditions, every part bracketed for replacement. Opt-in via a dialog checkbox that appears ONLY for a charter with no body (a GET preflight on the mark route answers that; the POST re-checks rather than trusting it, so a stale dialog can never overwrite writing). Written through `writeNoteContent`, never a `NotePayload` upsert — where a `CollaborationDocument` row exists a payload write is masked in any open editor and destroyed by that session's next store; a folder promoted moments ago usually has no Y.Doc, but "usually" is the assumption that path exists to remove. A failed scaffold degrades to the empty-charter warning, never to a failed promotion. `## Context` is not filler: STAGE2-CHARTER-RECIPE.md records the failure it prevents — a rubric scoring "against my profile" with no profile in the charter stalled the first production run outright ("I'd rather not fabricate Fit % values", 2026-09-04). Two conventions are described rather than demonstrated: `model:` (live only as a section's first line — a commented example would arm itself the moment the paragraph above it was deleted) and `[[wiki-links]]` (a literal example ships a dead reference). `charters:check` now round-trips the template through the real parser — phase count and titles, standing rules placement, a `Done when:` on every work phase, no armed `model:` directive, a genuinely-parsing output directive, no dead references — **mutation-tested five ways** (heading-level drift, reworded stop condition, broken output cue, smuggled `model:` line, literal wiki-link), each caught with its own message.

**D4 — an empty charter announced nothing.** A folder marked as a charter with an empty body and no children still told the model to "read its full content with getCurrentNote" — which returns `(empty note)`, one wasted step, after which the model went hunting for whatever the user must have meant. The explicit-attach path had said "this charter is empty" since it was written; the ambient path never learned to. It does now, and explicitly forbids substituting another charter.

**September 10, 2026** *(branch `feat/hocuspocus-awareness-presence`; typecheck / lint 151 (0 errors) / polling:check 17 files **0 awaiting audit** / polling:smoke 58 assertions across three suites / collab:schema:check green; **no migration, no TipTap schema change → no Hocuspocus redeploy**)*: **Collaboration presence — SSE deleted, dormant heartbeats stopped** (plan: `POLLING-DISCIPLINE-PLAN.md` Phase 1, D26–D27)

Deleted the per-document presence `EventSource`. Its server side re-queried Postgres every 10s for the life of the stream and, because an SSE response never completes, billed Vercel provisioned memory the whole time — the same double charge that made the conversations stream expensive. Removed with it: the in-process pub/sub that fed it (`subscribeCollaborationPresence` / `notifyCollaborationPresence` and the global listener store), which only ever delivered same-instance events, which is why the 10s interval existed underneath it.

**The plan's checklist was wrong, and following it would have shipped a silent regression.** It said to subscribe to `awareness.on("change")` in place of the SSE — but awareness only describes peers you are *already connected to*. A document sleeping in `localOnly` has no provider, therefore no awareness, so nothing would ever tell it a collaborator arrived: `applyRemotePresenceSessions` calling `promote()` is the **wake path**, and awareness structurally cannot see the thing that would wake it. Display and wake are different questions, so they now use different signals — awareness owns the badge while connected (unchanged), and the wake signal rides the shared presence poller already batching for the tab strip, at ~zero marginal cost.

**D26 — deep-dormant tabs stop heartbeating entirely.** A single note left open in a background tab kept Neon awake indefinitely: the dormant tier beat every 5 minutes, exactly the autosuspend threshold, so one forgotten tab undid the engagement work. Safe to stop because `ACTIVE_TRANSPORT_STATES` already excludes dormant sessions from the Note Window edit gate — the writes were feeding a filter. The record is now released (`surfaceCount: 0`) so collaborators see the session leave promptly instead of watching a stale grey badge for 8 minutes; it returns within a second of the tab coming back. The dangerous half was **resuming**, not stopping: two activity paths (the Y.doc update handler — the user *typing* — and `updateConsumer`) stamped `lastActivityAt` without rescheduling, relying on the next 5-minute beat to re-evaluate. With no timer left, a user who returned and started working would have stayed invisible to collaborators indefinitely. `noteLocalActivity` now stamps and resumes together.

D5 revisited: active heartbeat 10s → 20s (two beats land at 40s, inside the 45s prune window, so one drop is still survivable). The cadence decision moved to `presence-cadence.ts` as a pure function so it can be tested at all; the runtime is a stateful singleton wired to DOM/IndexedDB/Y.js. All three long-standing `unreviewed` registry files audited — `MarkdownEditor`'s 1s tick reads Y.js awareness off the in-memory provider, no network — leaving `polling:check` at **0 awaiting audit for the first time**.
...
The full arc from AI runs that emit markdown to runs that land **database rows**. (1) **Capture (P1+P2)** — `captureTo` on the iteration proposal card is the consent moment: code-level preflight (schema/options/descriptions/dedupe), then admitted items land as rows inside `record_item_result` — validate-ALL-before-any-write, upsert by url identity, admission rules enforced; new mutation-tested `capture:check` gate. (2) **Quests (P4a–c)** — Charter → Quest → Sitting: charter-owned master ledger (one row per quest, mandatory contentLinks), AI-sculpted quest ledgers with cross-sitting memory (rejects included — already-scored items skip with zero re-reads AND zero re-records), durable run state in ledger metadata with compaction-proof budget fallback, batch-boundary context folding whose UI (pills + batch gallery cards) mirrors exactly what the model retains, and the standing **artifact-reuse policy**: one quest log + one quest ledger per quest, homed beside the charter, no roll-up notes. (3) **Stage 2 & generation (P3+P5)** — `source:"database-rows"` refinement passes iterate the rows themselves and stamp back in place; `propose_output_database` lets the AI design the capture table (description required on every column) with the user's Apply creating it beside the charter; charter-registry jurisdiction lets a continued quest reach its linked tables without a fresh mention; first-class captureTo card rendering, pre-approval charter guard, session-cumulative token popover, D8 mid-run single-column grace, stage-2 recipe doc. §9.1 cost verification **measured**: dedup short-circuits to zero reads, folding holds per-request input flat (~21–26k while reads accumulate), cache ratio 61–93% across folded batches, quest runs produce zero redundant notes. Also rode: OpenAI `store:false` transcript fix (#193), tab-menu viewport fix (#188), W36 squash plan.

**August 30, 2026** *(updated Sep 1: merged **PR #201** — grew through owner smoke-testing into the full Database II arc: longText/date/checkbox suites, Images column (file+imageOnly config specialization), select cell pickers with inline option creation, File=uploads doctrine enforced server-side, drag/drop/paste uploads, overwrite-in-place (`overwriteFileNode`), editable DB title + breadcrumbs, click-away peek, header check-all, AI `create_shortcut` + `alsoShortcutTo` + `create_docx` overwrite mode, follow-ups restraint + anchored ✨ popover, chat copy-link; 40-line smoke checklist verified by owner on the PR)*: **Database II — number formatting, column resize, context rail, AI option proposals** (typecheck / lint / ai:drift:check / full build green; no migration, no TipTap schema change → **no Hocuspocus redeploy**)

Four database-feature improvements in one pass. (1) **Number column formatting** — `numberFormat` existed in `DataColumnConfig` but was read by nothing; now `formatNumberCell`/`cellToDisplayText` render currency (Intl, per-currency decimals, 20-code picker), percent (appends `%` to the raw value, never multiplies), decimal places and optional thousands grouping — display-only, because `cellToText` feeds search/CSV/filters/digest and must stay raw. Config UI is a new number branch in `ColumnMenu` with a live preview. (2) **Drag-to-resize column widths** — `ColumnPref.width` existed unwired end-to-end; a pointer-driven grip on the header (draggable-child trick so the reorder drag can't claim it) threads a memoized width map through header + all seven grid cell sites, persisted per view via the views PATCH, which now accepts a clamped `columnPrefs` map (locked-view guard included). (3) **Database context rail** — new "Database" right-sidebar tab (`database-tab`, `contentTypes: ["data"]`): column list with inline editing via `ColumnEditForm`, extracted from the header popover so the two surfaces render the same editor; `AddColumnButton` reused as-is; grid ⇄ rail sync over a `dg:data-schema-changed` CustomEvent with source tags (skip-own-echo); editing gated on `accessLevel === "owner"` to match the route's `canAlterSchema`. (4) **`propose_column_options` AI tool** — the flashcards proposal contract applied to select/multiSelect/status vocabularies: the tool validates against live schema and writes NOTHING; `ColumnOptionsProposalCard` (checkboxes + editable labels, localStorage already-applied flag) re-reads live config on Apply, merges with fresh option ids, PATCHes, and broadcasts the schema-changed event. Classified in `DATA_TOOL_IDS` — `ai:drift:check` green at 60 tools.

**August 29, 2026**: **File shortcuts** (branch `feat/file-shortcuts`, merged **PR #197** Aug 30; typecheck / lint 151 / full build green; ⚠ **migration `20260828120000_add_shortcut_content_type` must `migrate deploy` before any non-local env**; no TipTap schema change → **no Hocuspocus redeploy**; owner smoke passed through creation, mirroring, removal and picker parity)

A `shortcut` ContentType plus a `ShortcutPayload` pointer table gives the tree its first true alias: one canonical node, N pointers. Every prior "show a node elsewhere" mechanism (`ownedByNoteId`, people mounts, the reference drawer) *relocates* the single real node; a shortcut is its own ContentNode row, so it carries its own title, parent, order, icon and soft-delete while opening its target. `targetContentId` is nullable with `onDelete: SetNull`, which distinguishes the two broken states for free — a **trashed** target leaves the pointer intact so restoring heals every shortcut aimed at it with no cascade and no repair job, while a **purged** one nulls the pointer permanently.

**Folder shortcuts mirror recursively, view-only** ("safety-first recursive folder mirroring"). Three properties make that safe and each defends a distinct failure: **lazy** (a level is built only when its row is expanded, so a shortcut-to-A living inside A costs one level per click instead of hanging the tab), **path-scoped ids** (`smirror:<shortcutId>/<realId>`, extended per level, so the same content mirrored twice — or reached again through a cycle — still yields distinct rows, which react-arborist requires for selection, expansion, scroll-to and drop positions), and a **depth cap** (expansion state is persisted, so replayed open-ids could otherwise build an enormous tree with no click to pace it). Mirror rows reuse the reference block's wash and rail, are never drag sources, and withhold every mutating menu action — their synthetic ids would otherwise either miss or hit the real content from a place the user did not think they were editing. Per-shortcut **Hide nested shortcuts** trims the shortcuts-within-shortcuts case.

**Nothing is ever stored under a shortcut**, enforced in create and move alike; dropping onto a folder-shortcut or a mirrored folder **forwards** to the real folder before anything optimistic or networked happens. Shortcuts may be stored *under* anything, including content that hosts nothing else. Removal is one click with no warning — verified non-destructive rather than assumed: both DELETE cascades key off `ownedByNoteId` / `ContentLink`, which a shortcut structurally never has. Mixed selections still confirm, with shortcuts labelled so "• Workflows" cannot read as the folder being trashed.

New `pnpm shortcut-mirror:check` gate (in `build`) pins identity, laziness, id-uniqueness under cycles, drop forwarding and nested-shortcut hiding — mutation-tested against four deliberate regressions, none of which tsc, eslint or the production build can see.

**Bundled fixes.** (1) Creating anything with an **orphan-promoted** row selected failed with "Cannot add content to deleted parent" — deleting a folder leaves its children live holding a `parentId` that names the tombstone, and the tree promotes them to root; latent in the external-link path long before shortcuts, fixed in both. (2) **Icon gaps**: `workflow` and `hope` had no case in the file tree's `getIcon`, and `headers/tab-icons.ts` (the canonical tab map) and `folder-views/ListView.tsx` were missing up to six types each — all rendering generic documents. `ContentTreePicker`'s `TypeIcon` had the same gap, and its `DEFAULT_ELIGIBLE_TYPES` (tuned for "windowable") silently made databases unpickable. (3) The picker gained **press-and-hold to pick** and the double-click toggle-reversal, matching the tree's gesture model.

**August 27, 2026** *(updated Aug 28: auto-sync-on-open added — persisted column→deck links in user settings (`flashcards.dataDeckLinks`, server-managed, no migration); the review queue and the database viewer reconcile linked decks on open, staleness-gated on DataRow/DataColumn `updatedAt`; deleted rows keep their cards active by owner decision)*: **Database → flashcard deck conversion** (branch `feat/data-to-deck`, merged **PR #195** Aug 30; typecheck/lint/build green; no migration, no TipTap schema change → no Hocuspocus redeploy; owner smoke PASSED Aug 29 after trigger fix — panel Play fetches `GET /api/flashcards`, not `/queue`, so the sync hook moved into the list route; added per-deck ↻ force-sync button, dynamic Create/Sync labels, link-prefilled dialog, client link cache `state/data-flashcards-links-store.ts`)

Pick two columns of a database and `POST /api/flashcards/from-data` turns each row into a card — front column → question, back column → answer, column names as labels, deck path defaulting to the database title (`ensureDeckPath` find-or-creates). Re-running is a **sync, not a re-import**: cards carry their source `DataRow.id` in `Flashcard.noteId` (both UUIDs, no schema change), matched by `(deckId, noteId)`, and content-only updates leave FSRS scheduling untouched — fixing a typo in a row never resets review progress. Cell text goes through the CSV export's hydrated read-model (`cellDisplayValue`, now exported), so relation/person/lookup columns become titles/names, not ids. Entry points: file-tree context menu on `data` nodes + database viewer header button, both extension-gated; dialog mounts globally alongside the playbook dialog. **Bundled fix**: `loadTable` now populates `DataTable.query` per its documented contract — query-mode tables previously returned zero rows to every `table.query` consumer, including silently-empty CSV vault exports. — folder-derived sub-workspaces, nestable 3 layers deep** (branch `feat/workbenches`, PR pending; typecheck/lint/build green; ⚠ **migration `20260826120000_workbenches` must `migrate deploy` before any non-local env**; ⚠ **`vercel.json` cron entry needed for `/api/cron/dormant-workbenches`**; owner smoke pending)
