...
**September 4, 2026 (evening)** *(merged **PR #210** "Release 6: Chat control panel & workspace write-guard (AI 3.8)"; typecheck / lint 151 / full build green; no migration, no Hocuspocus redeploy; owner iterated the UX live across ~a dozen refinement passes before merge)*: **AI 3.8 + chat-surface UX round + workspace write-guard**

The 3.x roadmap's 3.8 milestone plus everything the owner's live passes surfaced. (1) **ChatControlPanel** — one labeled home (settings-row layout, tooltip hints, click-away via DOM-order portal test) for Target folder, Target output, Lock selected model (Lock/LockOpen icons), and Context (Feather icon); rail condensed to the model picker; header target chips retired; TargetFolderChip's menu portaled. (2) **"Status is text, boxes are content"** (new `guides/ui/CHAT-UI-CONVENTIONS.md`): reasoning disclosures (all four provider renderers), Working and Thinking cues all borderless inline text — frames reserved for content cards. (3) **Chat-surface fixes**: expand-to-full-view keeps inherited target chips (`deriveTargetSeed` single-source); eternal "running…" tool spinners settle as honest interruptions (render-time invariant covering every interrupt path); provider strip collapses to the single overflow menu when the mobile nav is engaged; cmd/ctrl-click in the file tree is multi-select only (never opens content); Export Chat icon-only; tool bubbles show content-aware token estimates on hover (±20%, labeled). (4) **Chat fork replaces blank duplicate** — the duplicate route forks the backing Conversation (full transcript) and mac-style increment naming replaces "(Copy)" chains; duplicate is single-select only. (5) **Workspace null-base write-guard** — stale writers reconcile instead of clobbering fresh drag-and-drop (closes the D+D revert mechanism). 3.x line now complete except T6 (acquisition explainer doc).
