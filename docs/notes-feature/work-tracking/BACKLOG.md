...
- [ ] **Trigger nodes for Trellis (user-requested 2026-07-13)** — a third node class (`execution: "trigger"`, exactly one, must be the entry): Manual (declares a Run input form — replaces the URL prompt), Page Capture (extension targets workflows by trigger type instead of the most-recently-updated heuristic), Schedule (cron dispatch), Webhook (per-workflow token URL), Content Event (note created/updated in a folder — the garden-native one). Triggers shape `{{input.*}}` and give the builder typed input hints. This is the tray.io "initiating connector" pattern; n8n/Zapier/Make all converge on it.
