...
- [ ] **Auto-context: per-user daily spend ceiling — NEXT UP (user-requested 2026-07-16)** — per-invocation caps bound each drain, but nothing bounds a day's total across drains. Add a counter row (userId, date, generationCalls) checked in the refresh engine's gate stack; surface "budget exhausted" like the settle skip. Build right after the V1.1 items settle.
