...
- [ ] **Buried background tabs are live writers — no viewing required** (resolved diagnosis 2026-09-04: owner found old, long-unviewed 3015 tabs; the workspace record went quiet once closed). The mechanism needs no user interaction: dev HMR delivers Fast Refresh to EVERY open tab over the websocket, re-running effects — including the shell controller's debounced persist — so a buried tab pushes its hours-old pane arrangement (stale base → 409-adopt-rewrite, or wiped module maps → null-base unconditional overwrite) and the active window's "saved by another tab" refresh applies it, reverting fresh drags. Prod has no HMR, so buried tabs stay inert there. Fan-in work should make a client's writes carry a liveness/recency signal (e.g., refuse or down-rank saves from clients whose snapshot base is far behind) rather than trusting every debounced writer equally.
