...
- [ ] **The reverting writer is UNIDENTIFIED — owner confirms a SINGLE 3015 session** (multi-tab theory withdrawn 2026-09-04). Surviving single-client hypotheses, none confirmed: (a) restore-from-URL — the URL query carries a layout snapshot (`tabs_top_left=…`) that predates the drag; any cold-load/refresh path that honors the URL tiebreaker would re-apply the stale arrangement; (b) dev Fast Refresh re-arming the shell controller's debounced persist against a stale snapshot / wiping the module-scope `lastAppliedUpdatedAt` maps (null base → unconditional overwrite); (c) a non-tab second client — the browser-extension side panel embed counts as a live writer if open on localhost. Diagnostic that settles it: watch `ContentWorkspace.updatedAt` + `paneState` during a repro and attribute each write. Hard refresh reliably recovers; prod unaffected.
