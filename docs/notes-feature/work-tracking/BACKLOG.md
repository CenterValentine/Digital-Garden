...
- [x] **Explicit attachment recognition for pasted/unsectioned playbooks** — FIXED 2026-07-23 after owner smoke: literal SKILL.md stored in ordinary TipTap paragraphs previously parsed as `0 phases`, so the route silently omitted the attached context and the model searched by topic. Markdown-like headings/frontmatter now parse into phases, unsectioned content becomes one implicit phase, empty attachments stay explicitly identified, and a resolved attachment removes `search_playbooks` for that turn. Covered by the build/CI `playbooks:check` gate.
