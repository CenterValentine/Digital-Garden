...
- [ ] **Auto-context: incremental roll-up patching (mechanism D)** — v1 folder regens are compositional (children's summaries only) which is cheap at <50 children; for large folders, add existing-rollup + changed-children patching with a forced full rebuild every K patches (drift guard). Designed 2026-07-16, deferred as marginal at current folder sizes.
...
- [ ] **Auto-context: per-folder opt-out** — global mode only in v1 (settings dialog). Junk-drawer folders may deserve an exclusion flag on the folder itself.
