...
**Ledgers at mark, bound-chat attachment, system columns (owner directives 2026-09-11 — `feat/charter-ledgers-on-mark`):**

- **The master ledger is minted when the charter is marked**, not on the first approved run. D10 stays as the self-heal path (a wiped or trashed stamp re-mints on the next run); re-marking an older charter is the backfill. It lands as *referenced* content — `role: "referenced"`, `ownedByNoteId = charter` — so the charter's reference chip shows it from the moment of marking. Quest ledgers are referenced to the charter the same way. A **folder charter homes its artifacts inside itself**; a note charter homes them in its containing folder. Motivation: with nothing visible at mark, the owner built a "charter database" by hand in prod (Career Hunt Charters — an orphan table nothing reads).
- **A chat bound to a charter is attached to it.** The former "ambient awareness" tier (a hint, no `activeCharter`) is gone: a charter's side chat loads standing rules + current phase and binds the quest machinery exactly as `/charter` does. Attachment is not execution — the model still runs only when asked. The client mirrors the binding in the composer chip and the persisted `data-charter` part; an explicit pick still wins. The rooted-execution regex also accepts the word *charter*.
- **System columns.** The machinery core of both ledgers carries `DataColumnConfig.system: true` (a config specialization — no migration). Locked: name, type, options, existence. Free: description, position, cells. Enforced in `updateColumn` / `softDeleteColumn` (server-authoritative; compared against the live row so an unchanged grid round-trip passes), the column PATCH/DELETE routes, the grid column menu (name disabled, options editor hidden, lock glyph in the header) and `propose_column_options` (refuses). Sculpted (AI-shaped) quest-ledger columns are NOT system — the user owns those.
- **Ledger writes are checked.** `ensureQuest` and `recordQuestItem` refusals (a needed column gone or retyped) now log and surface — `record_item_result` returns `questWarning` — instead of reporting an item ok while its row was never written. A master row with no live ledger link (typed into the grid, inserted by the AI, or its ledger trashed) **self-heals into a real quest** on its first run instead of returning null.
- **Attached-charter context names the ledger** (title + id), the quest → sitting shape, and the tools allowed to touch it; the iteration guidance in the system prompt says the same. Starter placeholder headings ("Phase 1 — [name the first phase]") are counted at mark (`countStarterPlaceholderPhases`, CI-pinned) and reported in the mark toast and the attached context — never auto-deleted.

**AI tools that may update a ledger (the answer to "what tool does the job"):**

| Ask | Tool | Protection |
|---|---|---|
| add fields to a ledger | `propose_database_columns` (card) | add-only by construction; existing names reported present |
| add or change a vocabulary on a *user* column | `propose_column_options` (card) | refuses system columns |
| a criterion that emerged mid-run (quest ledger) | `add_quest_ledger_column` (no card, D8) | new column only; restored to the mid-run tool diet |
| change a cell | `update_row` | unchanged — cells are never locked; the machinery re-derives what it owns at sitting close |
| start a quest | none — approve `propose_item_iteration` with `quest` | a hand-added master row self-heals on its first run |

**Proposed, not built — `propose_column_changes`.** Rename / retype / delete of *user* columns has no AI path; PR #220 chose add-only deliberately ("those stay the user's own action in the grid"). If the owner wants the AI to reshape a ledger, the right shape is a **new** card-gated tool rather than widening `propose_database_columns`: one proposal of `{ column, op: rename | retype | delete, to? }` entries, diffed against the live schema, **system columns rejected at propose time** (the same `config.system` check), retype limited to the lossless pairs the column route already permits, delete soft (restorable), the card the consent moment. A separate tool keeps the add-only tool's contract — which every existing prompt relies on — unchanged. Unscheduled; tracked in §10.
...
- **`propose_column_changes`** (rename / retype / delete of *user* columns, card-gated, system columns rejected at propose time) — proposed 2026-09-11 in §3.6; unscheduled, pending the owner's call on whether the AI should reshape ledgers at all.
- **Table-level ledger marker** — `insert_rows` into a master ledger still creates a row without a ledger link (it self-heals on the next run since 2026-09-11, but the AI is not told the master is special). A `DataPayload.source.system` marker would let the tools refuse or route; deferred until a case demands it.
- **Starter scaffold is never auto-removed** — placeholder phases are counted and reported (mark toast, attached context, `charters:check`); deleting them stays the user's action.
