...
- **Run gallery card (owner, 2026-09-01; RESHAPED 2026-09-03 — one gallery of BATCHES, not one card per batch)**: the owner's clarified expectation is one level up from the original sketch — the whole run collapses to **ONE gallery unit in the transcript that pages ACROSS batches** (Batch 1 · Batch 2 · …), because per-batch cards stacked vertically re-create the scroll bloat folding exists to kill at job-hunt scale (50 items, 5+ batches). Shape: run-level header (objective · recorded/budget · qualified count · cost · quest ledger/log chips) + batch pages, each page an internal **carousel of item panels** (label · fit % · qualified badge · verdict snippet · row chip → DataRowPeek); expand a panel for that item's raw exchanges. The in-flight batch (no checkpoint yet) renders live and unfolded, then docks into the gallery when its checkpoint records. Pure rendering over already-persisted parts, grouped between the existing checkpoint anchors — no data-model change. **The collapsed view IS the model's retained view** (§6.1): default presentation and model context show the same folded shape; expansion is user-initiated, temporary, and reveals what the model dropped. A capture-failed item's panel shows its failure reason inline. *Build status: the fold mechanism + minimal per-part pill mirror shipped in PR 3 (#204); the gallery is the presentation layer riding PR 4 with the other card-surface work (captureTo card, token popover) — pills remain the fallback for folded parts outside any batch.*
