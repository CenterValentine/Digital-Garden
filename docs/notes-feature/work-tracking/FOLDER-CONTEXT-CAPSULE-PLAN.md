...
| B2 | No refresh mutex exists (gen-lock is the anti-feedback-loop rule, not a lock) | Concurrent gates (two tabs; mention + on-access + cron) double-spend on the same scope | Per-(owner, folder) Postgres advisory lock (`pg_try_advisory_xact_lock`) around the scoped refresh — in-process locks don't survive serverless isolates. Loser polls coverage until budget expiry | 2 |
