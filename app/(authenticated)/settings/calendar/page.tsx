...
import type { CalendarConnectionDTO, CalendarSourceDTO } from "@/lib/domain/calendar/types";
...
export default function CalendarSettingsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      const [connectionsResponse, bootstrapResponse] = await Promise.all([
        ...
        fetch(`/api/calendar/bootstrap?start=${encodeURIComponent(new Date().toISOString())}&end=${encodeURIComponent(new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString())}`),
      ]);
      const [connectionsJson, bootstrapJson] = await Promise.all([
        parseJsonResponse<ConnectionsResponse>(connectionsResponse, "Failed to load calendar connections"),
        parseJsonResponse<BootstrapResponse>(bootstrapResponse, "Failed to load calendar workspace"),
      ]);
  ...
  const handleGoogleConnect = async () => {
    window.location.href = "/api/auth/google?redirect=/settings/calendar";
  };
  ...
  const handleSourceToggle = async (sourceId: string, visible: boolean) => {
    ...
    setSources((current) => current.map((source) => (source.id === sourceId ? updatedSource : source)));
  };
  ...
  const handleDefaultViewChange = (value: NonNullable<UserSettings["calendar"]>["defaultView"]) => {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Connect Google Calendar, subscribe to iCal feeds, and set defaults for the in-app calendar workspace.
        </p>
      </div>
        ...
        <div className="mt-5 grid gap-6 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(event) => handleDefaultViewChange(event.target.value as NonNullable<UserSettings["calendar"]>["defaultView"])}
            ...
            <Input
              ...
              onChange={(event) =>
                void setCalendarSettings({
                  defaultEventDurationMinutes: Number(event.target.value) || 60,
                })
              }
          ...
          <div className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.04] px-4 py-3">
            <div>
              <p className="font-medium text-white">Show weekends</p>
              <p className="text-sm text-gray-400">Keep Saturday and Sunday visible in FullCalendar.</p>
            </div>
            <Switch
              checked={calendar?.showWeekends ?? true}
              onCheckedChange={(checked) => void setCalendarSettings({ showWeekends: checked })}
            />
          </div>

          <div className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.04] px-4 py-3">
            <div>
              <p className="font-medium text-white">Show declined events</p>
              <p className="text-sm text-gray-400">Hide declined Google events if you want a cleaner calendar.</p>
            </div>
            <Switch
              checked={calendar?.showDeclinedEvents ?? true}
              onCheckedChange={(checked) => void setCalendarSettings({ showDeclinedEvents: checked })}
            />
          </div>
        </div>
      </section>
      ...
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">Manage remote calendars and refresh their sync state.</p>
          </div>
        ...
        <div className="mt-5 space-y-3">
          {remoteConnections.map((connection) => (
            <div key={connection.id} className="rounded-xl border border-white/5 bg-white/[0.04] px-4 py-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-medium text-white">{connection.displayName}</p>
                  <p className="text-sm text-gray-400">
                    {connection.provider} · {connection.status.replace(/_/g, " ")} · {connection.syncStatus}
                  </p>
                  {connection.lastSyncError && (
                    <p className="mt-1 text-sm text-red-300">{connection.lastSyncError}</p>
                  )}
                </div>
                {(connection.provider === "google" || connection.provider === "ical") && (
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={() => void handleGoogleSync(connection.id)}>
                      Sync now
                    </Button>
                    <Button variant="ghost" onClick={() => void handleDeleteConnection(connection.id)}>
                      Remove
                    </Button>
                  </div>
                )}
          ...
          {remoteConnections.length === 0 && !isLoading && (
            <p className="text-sm text-gray-400">No calendar connections yet.</p>
          )}
        </div>

        <div className="mt-6 grid gap-3 md:grid-cols-[1fr_1.4fr_auto]">
          <Input
            value={icalName}
          ...
          <Input
            ...
            className="border-white/10 bg-white/5"
          />
          <Button onClick={() => void handleCreateIcal()} disabled={!icalUrl}>
            Add iCal Feed
          </Button>
        </div>
      </section>
      ...
      >
        ...
        <p className="mt-1 text-sm text-gray-400">
          Create app-native calendars for planning that does not need an external provider.
        </p>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-[1fr_auto]">
          <Input
            ...
            className="border-white/10 bg-white/5"
          />
          <Button onClick={() => void handleCreateLocalCalendar()} disabled={!newLocalCalendar.trim()}>
        ...
        <div className="mt-5 space-y-3">
          {localSources.map((source) => (
            <div key={source.id} className="flex items-center justify-between rounded-xl border border-white/5 bg-white/[0.04] px-4 py-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">
                    {source.visible ? "Visible" : "Hidden"} · {source.isPrimary ? "Primary" : "Secondary"}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <Switch
                  ...
                  onCheckedChange={(checked) => void handleSourceToggle(source.id, checked)}
                />
                {!source.isPrimary && (
                  <Button variant="ghost" size="sm" onClick={() => void handleDeleteSource(source.id)}>
                    Remove
                  </Button>
                )}
              </div>
