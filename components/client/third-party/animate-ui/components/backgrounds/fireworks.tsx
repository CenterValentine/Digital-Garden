...
function createFirework(
  ...
  targetX?: number // Optional: if provided, calculate angle to hit this X position
): FireworkType {
  // Calculate angle based on target position if targetX is provided
  // Otherwise use random upward angle for automatic fireworks
  let angle: number;
  if (targetX !== undefined) {
    // Calculate angle to travel from (x, y) to (targetX, targetY)
    const dx = targetX - x;
    const dy = targetY - y;
    angle = Math.atan2(dy, dx);
  } else {
    // Random upward angle for automatic fireworks
...
>(function FireworksBackground(
  ...
  React.useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      // This accounts for canvas scaling and positioning
      ...
      // Firework starts from bottom of canvas at the click's X position
      const startX = clickX;
      const startY = maxY; // Always start from bottom

      // Firework explodes at the exact click position
      const targetX = clickX;
      const targetY = Math.max(0, Math.min(clickY, maxY)); // Clamp to canvas bounds
      ...
      fireworks.push(
        createFirework(
          ...
          particleSize,
          handleExplosion,
          targetX // Pass targetX so firework travels directly to click position
        )
      );
    };
