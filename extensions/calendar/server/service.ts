...
async function syncGoogleEventToProvider(
  ...
  const account = await getGoogleAccount(userId);
  if (!hasGoogleScope(account?.scope, GOOGLE_CALENDAR_SCOPE)) {
    throw new Error("Google Calendar write permission is missing. Reconnect Google Calendar from Settings.");
  }
