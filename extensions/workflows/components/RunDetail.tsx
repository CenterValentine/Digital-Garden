...
/**
 * Run supervision detail — extracted from WorkflowsPanel so BOTH hosts can
 * mount it: the left-sidebar panel and the browser-extension embed viewer
 * (EmbedWorkflowClient). Session-authed fetches only; inside the embed
 * iframe the layout's fetch wrapper injects the x-embed-session header.
 */
...
export function StatusPill({ status }: { status: WorkflowRunStatusValue }) {
  return (
    <span
