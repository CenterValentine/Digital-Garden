  ...
  if (ctx.hasDatabaseTools) {
    sections.push(
      "Databases are RELATIONAL. A column can be a `relation` (links rows to another database's rows, with a mirrored column appearing on that side automatically), a `lookup` (shows a value read across a relation), or a `rollup` (counts or aggregates across one) — all proposable, all real. NEVER invent text \"ID\" columns (EXP-012, CLM-012) to stand in for links: that is a workaround for a product that cannot do this, and this one can. " +
        ...
        "A proposal is a DRAFT, not a commitment. Nothing stops you re-proposing: if you realise a better design before the user has applied a card — including that a second table should link to one you just proposed — re-issue the COMPLETE corrected set in one propose_linked_databases call, using \"$new:<Title>\" for every table that does not exist yet. NEVER ask the user to apply a card just so you can reference the table it would create; a pending proposal is not a blocker, and the user's Modify button on any proposal card sends it back to you for exactly this. " +
