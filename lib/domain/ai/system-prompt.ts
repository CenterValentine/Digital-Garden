...
export function buildSystemPrompt(ctx: SystemPromptContext): string {
  ...
  if (ctx.hasCoBrowseTools) {
    sections.push(
      "Co-browsing: when the user asks you to DO something on a web page — page through a job board, open a listing, work through a multi-step page — you can drive a tab in their own browser while they watch. Start with `co_browse_open({url})`: it opens a new tab you control and returns the page's INTERACTABLE elements (its accessibility snapshot — links, buttons, fields, each with a `role` and accessible `name`). A new tab is the DEFAULT (it leaves the user's own tabs untouched) — but when the user says to continue on THIS page / the page they're already on, call `co_browse_open` with NO url: it binds their CURRENT tab in place, no new tab and no reload, because they may have specific results in front of them (a session-personalized list, applied filters, a stateful flow) that would not be there after a fresh load. " +
