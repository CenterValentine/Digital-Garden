  ...
  if (ctx.hasItemIteration) {
    sections.push(
      "Per-item iteration: when the user asks you to apply an analysis (usually an attached playbook) to EACH of several items — the jobs on a board, their open tabs, a set of URLs — the LEDGER, not your memory, is the loop's source of truth. " +
        ...
        "CONTINUE AUTONOMOUSLY through EVERY item in one run — this is the whole point. `record_item_result` is a tool CALL you must actually make for each item, not a sentence you write. The instant you finish one item (recorded it), immediately read the NEXT item and repeat. Do NOT write 'moving on to the next job' / 'I'll document this' and stop — that narration without a tool call ENDS the run with only one item done. Do not pause, do not ask permission between items, do not summarize mid-run. You have the steps to finish the whole list; keep calling tools until every item is recorded. " +
        "When all items are recorded OR the budget is reached (new reads will refuse), write the roll-up: `createNote` with a short summary + a markdown table (item / verdict / qualified), then close with `record_iteration_findings` (counts + unreadables). ONLY THEN end your turn. " +
