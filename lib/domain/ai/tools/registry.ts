...
export function createBaseTools(ctx: ToolExecuteContext) {
  return {
    ...
    // Sprint 45: Snippet retrieval tool
    getSnippets: tool({
      description:
        "Search the user's knowledge snippets for facts, ideas, and notes. " +
        "Snippets are short text fragments the user has saved for reference. " +
        "Use this when the user asks about their saved knowledge or when you need context from their snippet library.",
      inputSchema: z.object({
        query: z
          .string()
          .optional()
          .describe("Search query to filter snippets by content or title"),
        categoryId: z
          .string()
          .uuid()
          .optional()
      ...
      execute: async ({ query, categoryId, limit = 20 }) => {
        ...
        if (snippets.length === 0) {
          return query
            ...
            : "The user has no knowledge snippets saved yet.";
        }

        const formatted = snippets.map((s, i) => {
          const title = s.title || s.content.split("\n")[0].slice(0, 60);
          return `${i + 1}. [${s.category.name}] ${title}\n   ${s.content.slice(0, 300)}`;
        });

        return `Found ${snippets.length} snippet${snippets.length !== 1 ? "s" : ""}:\n\n${formatted.join("\n\n")}`;
      },
    }),
  };
}
