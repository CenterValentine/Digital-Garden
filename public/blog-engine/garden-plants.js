...
(function (global) {
  ...
  function renderDandelion(svg,cfg){
    ...
    pts.forEach(function(v){
      ...
      ray.setAttribute('class','ray');ray.setAttribute('stroke-width',(0.5+depth*0.9).toFixed(2));ray.setAttribute('opacity',op);svg.appendChild(ray);
      ...
      for(var h=0;h<hairs;h++){
        ...
        pp.setAttribute('class','pappus');pp.setAttribute('opacity',(op*0.8).toFixed(2));svg.appendChild(pp);
      }
      var sd=document.createElementNS(SVGNS,'circle');
      sd.setAttribute('cx',sx.toFixed(1));sd.setAttribute('cy',sy.toFixed(1));sd.setAttribute('r',(1+depth*1.6).toFixed(1));sd.setAttribute('class','seed');sd.setAttribute('opacity',op);svg.appendChild(sd);
    });
    for(var d=0;d<3;d++){
