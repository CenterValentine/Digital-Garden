...
/* ── next block ── */
  ...
  (function(){
    ...
    function run(){
      ...
      el.innerHTML=
        ...
        '<div class="sprout">'+
          ...
          '<img class="treelogo" src="logo-neuron-tree.png" alt="">'+
        '</div>';
      host.appendChild(el);
    ...
    /* Play the intro once per session. Replay on a hard refresh (reload), but
       NOT when the user navigates back to home from a sub-page (/blog, /about…). */
    function introShouldPlay(){
      try{
        var nav=performance.getEntriesByType&&performance.getEntriesByType('navigation')[0];
        var isReload = nav ? nav.type==='reload'
                           : (performance.navigation && performance.navigation.type===1);
        if(isReload) return true;                      // reload / hard refresh → replay
        return !sessionStorage.getItem('dvIntroSeen'); // first view this session → play
      }catch(e){ return true; }                        // if anything fails, don't hide the intro
    }
    if(introShouldPlay()){
      try{ sessionStorage.setItem('dvIntroSeen','1'); }catch(e){}
      setTimeout(run, 480);
    } else {
      /* Skip straight to the resting day state the intro would have left behind. */
      if(window.__setTheme) window.__setTheme('light');
  ...
  (function(){
    ...
    function doWhitewash(idx){
      ...
      setTimeout(function(){
        ...
        if(key==='yarrow'){ window.location.href='/about'; return; }
        if(key==='willow'){ window.location.href='/results'; return; }
        if(key==='hawthorn'){ window.location.href='/resume'; return; }
        if(key==='dandelion'){ window.location.href='/blog'; return; }
        if(key==='maple'){ window.location.href='/hobby'; return; }
        if(window.M44LeafConnect){ M44LeafConnect.openLeaf(key, originEl); return; }
        /* no handler available — undo zoom so garden returns to normal */
